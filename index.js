...
const UserClient = new MongoClient(config.nearcordKey);
  ...
  if (interaction.isButton()) {
    if (interaction.customId.startsWith('complete_second_mission')) {
      ...
      if (member) {
        ...
        interaction
          .reply(`${interaction.user} escapes the frostbite`)
          .then(() => {
    ...
    if (interaction.customId === 'complete_third_mission') {
      const twittycordUserData = await UsersCol.findOne({
        'social.discord': interaction.user.id,
      });

      let twitterId = twittycordUserData.social.twitter;
      ...
      while (continueFollowersFetching) {
        ...
        const followerIds =
          followers?.data?.map((follower) => follower.id) || [];
        ...
        console.log(
          `nextToken: ${nextToken}, continueFollowersFetching: ${continueFollowersFetching}`
        );
      }
      ...
      if (member) {
        ...
        interaction
          .reply(`${interaction.user} escapes freezing and enters the tundra.`)
          .then(() => {
            setTimeout(() => interaction.deleteReply(), 3_000);
          });
      }
    ...
    if (interaction.customId === 'complete_fourth_mission') {
      ...
      if (inviteCount < 2) {
        return interaction.reply({
          content:
            'Use a custom invite code to invite your two best friends! You have ' +
            inviteCount +
            ' mates right now',
          ephemeral: true,
        });
...
client.on('messageReactionAdd', (reaction, user) => {
  if (reaction.message.channelId === config.onBoardingChannelId) {
    ...
    if (userAccepted >= config.userCountThreshold) {
      return console.log(
        `More than ${config.userCountThreshold} have been accepted on the onboarding message.`
      );
    }
...
const sendOnBoardingMessage = () => {
  ...
  const messagesToDelete = Array.from(sentOnBoardingMessages.entries()).filter(
    (m) => m[0] < olderThan
  );
...
const makeSureContentExits = () => {
  client.channels.cache
    .get(config.firstMissionChannelId)
    .messages.fetch()
    .then((messages) => {
      if (messages.size === 0) {
  ...
  client.channels.cache
    .get(config.secondMissionChannelId)
    .messages.fetch()
    .then((messages) => {
      if (messages.size === 0) {
        const embed = new Discord.EmbedBuilder()
  ...
  client.channels.cache
    .get(config.thirdMissionChannelId)
    .messages.fetch()
    .then((messages) => {
      if (messages.size === 0) {
        const embed = new Discord.EmbedBuilder()
  ...
  client.channels.cache
    .get(config.fourthMissionChannelId)
    .messages.fetch()
    .then((messages) => {
      if (messages.size === 0) {
        const embed = new Discord.EmbedBuilder()
  ...
  client.channels.cache
    .get(config.fifthMissionChannelId)
    .messages.fetch()
    .then((messages) => {
      if (messages.size === 0) {
        const embed = new Discord.EmbedBuilder()
  ...
  if (message.channelId === config.firstMissionChannelId) {
    ...
    if (message.content === config.firstMissionAnswer) {
      ...
      if (member) {
        ...
        message.channel
          .send(`${message.author} moves out of the cold`)
          .then((m) => {
      ...
      message.channel
        .send(
          `${message.author} looking at the padlock, notices that the 2, 6, 4, and 9 digits click differently. The universe needs answers. `
        )
        .then((m) => {
          setTimeout(() => m.delete().catch(() => {}), 3_000);
        });
  ...
  if (message.channelId === config.secondMissionChannelId) {
    ...
    if (message.content.includes('<:35:1004373977004789820>')) {
      ...
      if (member) {
        ...
        member.roles.add(config.thirdMissionRoleId);
        message.channel
          .send(`${message.author} escapes the frostbite`)
          .then((m) => {
            setTimeout(() => m.delete().catch(() => {}), 3_000);
          });
      }
      ...
      message.channel
        .send(
          `${message.author} Dancing is not made of writing. You have to move and search.`
        )
        .then((m) => {
          setTimeout(() => m.delete().catch(() => {}), 3_000);
        });
